const STORAGE_KEY = "casaMarketStoreV3";
...
function submitOrder(event) {
  ...
  const order = {
    id: uid("order"),
    code: String(store.orders.length + 1).padStart(4, "0"),
...
function renderAdminOrders() {
  ...
  els.adminOrdersList.querySelectorAll("[data-delete-order]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      store.orders = store.orders.filter(order => order.id !== button.dataset.deleteOrder);
      saveStore();
      renderAdminOrders();
      renderMyOrders();
    });
